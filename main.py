...
def add_subjs_argument(parser, arg_name, help):
    parser.add_argument(arg_name, metavar='SUBJ', nargs='*', type=str, help=help)

def add_archs_argument(parser):
    archs = ['mlp', 'cnn']
    parser.add_argument('--arch', default=archs, nargs='?', choices=archs,
...
def add_setups_argument(parser):
    setups = ['lp', 'hp']
    parser.add_argument('--setup', default=setups, nargs='?', choices=setups,
        help='''restrict corresponding analysis to 
...
def build_subparsers():
    ...
    parser.add_argument('--root', default='dataset', nargs=1,
    ...
    add_subjs_argument(preproc, '--missed',
    ...
    add_subjs_argument(preproc, '--head_mov',
    ...
    add_subjs_argument(preproc, '--shift_crop',
        '''for every 'SUBJ' subject, load estimated head movements and 
        with artificially added sensor shifts, crop appropriate image region
        to get 'inside-headset-like' close eye captions and save them to
        'ROOT\\SUBJ\\images_crop' directory.
        Run on the whole dataset, if no subjects specified'''
    )
    add_subjs_argument(preproc, '--psog',
        '''for every 'SUBJ' subject, simulate PSOG sensor outputs and save them 
        together with eye-movement signal 
    ...
    ml.add_argument('--evaluate', default=False, action='store_true',
        help='''evaluate best architectures found during grid-search 
        for 'fine-tune' and 'from scratch' approaches on the whole dataset''')
    ...
    plot.add_argument('--boxplots', default=False, action='store_true',
        ...
        evaluation of 'fine-tune' and 'from scratch' approaches and save them to
        '.\\plots\\boxplots' directory''')
    ...
    plot.add_argument('--samples_distrib', nargs='*',
        ...
        distribution on the screen with attempt to resemble calibration data split
        and save them to '.\\plots\\samples_distrib' directory.
        Run on the whole dataset, if no subjects specified''')
