...
def run_feature_extraction(exp_path, sensor, fpath_sr=None, img_mode='blender'):
    ...
    for n, frame in tqdm(data_exp.iterrows()):
        _movCam = frame[['smh', 'smd', 'smv']].values.tolist()
...
class Sensor():
    ...
    def __init__(self, design):
        ...
        self.design = zip(*design['design'])
        self.design.append(kernels)
        self.design = zip(*self.design)
        ...
        self.sr_names = map(lambda x: 'sr_%d'%x, xrange(len(self.output_transition)))
